...
class Spotify:
    def __init__(self, sp_dc):
        ...
        self.access_token_cache_time = 0

    async def get_access_token(self):
        if time.time() - self.access_token_cache_time < 5:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(self.auth_url, headers=headers) as response:
                ...
                if response.status == 200:
                    ...
                    self.access_token_cache_time = time.time()
                    return self.access_token
                else:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(track_api_url, headers=headers) as response:
                ...
                if response.status == 200:
                    ...
                    raise HTTPException(status_code=response.status, detail={"message": "Failed to retrieve track details", "response": raw_response})
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers) as response:
                ...
                if response.status == 200:
                    ...
                    raise HTTPException(status_code=response.status, detail={"message": "Failed to retrieve lyrics", "response": raw_response})
